...
<template>
  ...
  <section class="ctD-wrap">
    <canvas class="ctD" ref="ctD" :style="styles" width="80" height="80">浏览器不支持Canvas,请升级或改用其它浏览器！</canvas>
  </section>
</template>
...
export default {
  props: {
    ...
    width: validatorNumber(60),
    height: validatorNumber(60),
    // 倒计时的总时间 (s)
    ...
    // 倒计时进行时的文本
    tiping: validatorFontStyle("倒计时", "#000"),
    // 倒计时结束时的文本
    tipend: validatorFontStyle("END", "#000"),
    // 倒计时圆盘背景颜色
    bgCir: {
      type: String,
      default: "#B2B2B2"
  ...
  methods: {
    ...
    staticTip() {
      ...
      this.clearCavans();
      this.drawCirBg();
      this.drawText(this.tipend, 20, 0, 0);
    }
  },
  mounted() {
    // init
